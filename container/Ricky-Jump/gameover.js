  ...
  static handleGameOver(data) {
    ...
    try {
      if (typeof sendScore === 'function') {
        ...
        window.sendScore(data.score);
      } else if (typeof globalThis !== 'undefined' && typeof globalThis.sendScore === 'function') {
        globalThis.sendScore(data.score);
      } else if (typeof ScoreAPI !== 'undefined' && ScoreAPI && typeof ScoreAPI.sendScore === 'function') {
        ScoreAPI.sendScore(data.score, data);
      } else {
        console.warn('[GameOverController] sendScore function is not available.');
      }
    } catch (e) {
      console.error('[GameOverController] sendScore error:', e);
    }
