...
def generate_booster_btn(icon_type, size=180):
    ...
    # Icon
    icon_color = (210, 220, 240, 255)
    if icon_type == "trash":
        # Trash bin
        bx, by = cx - 22*scale, cy - 32*scale
        draw.rounded_rectangle([bx + 4*scale, by + 16*scale, bx + 40*scale, by + 56*scale], radius=4*scale, fill=icon_color)
        draw.rounded_rectangle([bx, by + 8*scale, bx + 44*scale, by + 16*scale], radius=3*scale, fill=icon_color)
        draw.rounded_rectangle([cx - 10*scale, by, cx + 10*scale, by + 8*scale], radius=3*scale, fill=icon_color)
        # Vertical slots
        for sx in (-8*scale, 0, 8*scale):
            draw.line([(cx + sx, by + 22*scale), (cx + sx, by + 50*scale)], fill=(51, 58, 76, 255), width=int(3*scale))
    elif icon_type == "hammer":
        # Angled hammer matching video
        hx, hy = cx - 26*scale, cy - 34*scale
        # Hammer head
        draw.rounded_rectangle([hx, hy, hx + 52*scale, hy + 26*scale], radius=5*scale, fill=icon_color)
        # Hammer claw/back
        draw.polygon([(hx, hy + 6*scale), (hx - 10*scale, hy + 18*scale), (hx, hy + 24*scale)], fill=icon_color)
        # Handle
        draw.rounded_rectangle([cx - 7*scale, hy + 26*scale, cx + 7*scale, hy + 72*scale], radius=4*scale, fill=(180, 150, 120, 255))
    elif icon_type == "undo":
        # Curved undo arrow
        arc_r = int(28 * scale)
        draw.arc([cx - arc_r, cy - arc_r - 8*scale, cx + arc_r, cy + arc_r - 8*scale], 40, 310, fill=icon_color, width=int(10 * scale))
        ax = cx + arc_r * math.cos(math.radians(40))
        ay = cy - 8*scale + arc_r * math.sin(math.radians(40))
        draw.polygon([(ax, ay - 14*scale), (ax + 20*scale, ay + 2*scale), (ax - 2*scale, ay + 16*scale)], fill=icon_color)

    # Cost pill at bottom (Coin + 100)
    coin_y = h - 45 * scale
    draw.ellipse([cx - 44*scale, coin_y - 12*scale, cx - 20*scale, coin_y + 12*scale], fill=(255, 205, 40, 255))
    draw.ellipse([cx - 40*scale, coin_y - 8*scale, cx - 24*scale, coin_y + 8*scale], outline=(220, 160, 20, 255), width=int(2*scale))
    
    font = get_font(int(26 * scale), bold=True)
    draw.text((cx - 14*scale, coin_y - 18*scale), "100", font=font, fill=(255, 255, 255, 255))

    # Little '?' info badge on top right
    badge_x, badge_y = w - 26*scale, 26*scale
    draw.ellipse([badge_x - 14*scale, badge_y - 14*scale, badge_x + 14*scale, badge_y + 14*scale], fill=(26, 175, 155, 255))
    q_font = get_font(int(18 * scale), bold=True)
    draw.text((badge_x - 5*scale, badge_y - 12*scale), "?", font=q_font, fill=(255, 255, 255, 255))
...
def generate_trophy(size=120):
    ...
    # Base
    draw.rounded_rectangle([cx - 28*scale, cy + 32*scale, cx + 28*scale, cy + 45*scale], radius=4*scale, fill=(180, 140, 20, 255))
    draw.rectangle([cx - 8*scale, cy + 16*scale, cx + 8*scale, cy + 32*scale], fill=(220, 170, 25, 255))
    # Handles
    draw.arc([cx - 46*scale, cy - 36*scale, cx - 12*scale, cy + 10*scale], 60, 300, fill=(220, 170, 25, 255), width=int(8 * scale))
    draw.arc([cx + 12*scale, cy - 36*scale, cx + 46*scale, cy + 10*scale], 240, 120, fill=(220, 170, 25, 255), width=int(8 * scale))
    # Cup
    cup_pts = [
        (cx - 30*scale, cy - 40*scale),
        (cx + 30*scale, cy - 40*scale),
        (cx + 25*scale, cy + 4*scale),
        (cx + 12*scale, cy + 16*scale),
        (cx - 12*scale, cy + 16*scale),
        (cx - 25*scale, cy + 4*scale)
    ]
    draw.polygon(cup_pts, fill=(242, 192, 38, 255))
    draw.ellipse([cx - 30*scale, cy - 45*scale, cx + 30*scale, cy - 35*scale], fill=(255, 215, 60, 255))
    return img.resize((size, size), Image.Resampling.LANCZOS)
...
trophy = generate_trophy(120)
trophy.save(os.path.join(UI_DIR, "icon_trophy.png"))
