...
class Make7GamePortrait {
    ...
    loadSprites() {
        const paths = {
            slot: 'images/board_slot_dark.png',
            tile_1: 'images/tile_1.png',
            tile_2: 'images/tile_2.png',
            tile_3: 'images/tile_3.png',
            tile_4: 'images/tile_4.png',
            tile_5: 'images/tile_5.png',
            tile_6: 'images/tile_6.png',
            tile_7: 'images/tile_7_rainbow.png',
            rotate_arrows: 'images/icon_rotate_arrows.png',
            trophy: 'images/icon_trophy.png'
        };
    ...
    getTileColor(val) {
        const colors = {
            1: '#8e44ad', // Violet
            2: '#eb8c1e', // Honey Amber
            3: '#eb4b5f', // Coral Red
            4: '#2da8eb', // Sky Blue
            5: '#94c62d', // Lime Green
            6: '#f2c026', // Golden Yellow
            7: '#ffffff'  // Rainbow 7
        };
    ...
    drawHexTile(ctx, cx, cy, R, val, alpha = 1.0) {
        ...
        if (sprite && sprite.complete) {
            ...
            // Clean procedural wooden vector fallback
            const col = this.getTileColor(val);
            
            // 3D tactile bottom rim
            ctx.beginPath();
            for (let i = 0; i < 6; i++) {
                const a = (60 * i + 30) * Math.PI / 180;
                const px = cx + R * 0.96 * Math.cos(a);
                const py = cy + 4 + R * 0.96 * Math.sin(a);
            ...
            // Bold debossed black number matching new.png
            ctx.font = `900 ${R * 0.95}px 'Fredoka', 'Nunito', sans-serif`;
            ctx.textAlign = 'center';
            ctx.textBaseline = 'middle';
            
            // Subtle engraved shadow
            ctx.fillStyle = 'rgba(0, 0, 0, 0.45)';
            ctx.fillText(val === 7 ? '7' : val, cx, cy + 2);

            // Deep black number
            ctx.fillStyle = '#141416';
            ctx.fillText(val === 7 ? '7' : val, cx, cy);
        }
    ...
    renderBoard() {
        ...
        // 1. Draw Recessed Dark Wooden Sockets
        for (const cell of this.cells) {
            ctx.save();
            ...
            if (slotSprite && slotSprite.complete) {
                ...
                // Procedural carved dark wooden socket fallback
                ctx.beginPath();
                for (let i = 0; i < 6; i++) {
                    const a = (60 * i + 30) * Math.PI / 180;
                ...
                ctx.closePath();
                ctx.fillStyle = '#4a2914';
                ctx.fill();
                ctx.lineWidth = 2.5;
                ctx.strokeStyle = '#2c170a';
                ctx.stroke();

                // Triangular carved facet lines meeting at center
                for (let i = 0; i < 6; i++) {
                    const a = (60 * i + 30) * Math.PI / 180;
                    ctx.beginPath();
                    ctx.moveTo(0, 0);
                    ctx.lineTo(HEX_R * 0.94 * Math.cos(a), HEX_R * 0.94 * Math.sin(a));
                    ctx.strokeStyle = 'rgba(30, 15, 7, 0.6)';
                    ctx.lineWidth = 1.5;
                    ctx.stroke();
                }
            }
        ...
        for (const h of this.hoveredCells) {
            ...
            ctx.closePath();
            ctx.strokeStyle = '#f2c026';
            ctx.lineWidth = 4;
            ctx.stroke();
            ctx.restore();

            this.drawHexTile(ctx, h.cell.x, h.cell.y, HEX_R, h.val, 0.65);
        }
    ...
    openGameOver() {
        ...
        try {
            if (typeof sendScore === 'function') {
                ...
                globalThis.sendScore(this.score);
            } else {
                console.warn('[GameOver] sendScore function is not available.');
            }
        } catch (err) {
            console.error('[GameOver] sendScore error:', err);
        }
    }
