...
class DotPair {
  constructor(y, radius, spacing, isTop = true) {
    ...
    // State 0: Cyan is in center, Pink is offset
    // State 1: Pink is in center, Cyan is offset
    this.state = 0; // 0 or 1
    this.popProgress = 1; // Instant tactile pop on swap
  ...
  swap() {
    // Immediate position flip - zero delay / lag
  ...
  update(dt) {
    if (this.popProgress < 1) {
      this.popProgress = Math.min(1, this.popProgress + dt / 0.08); // Snappy 80ms pop return
    }
  }
  ...
  draw(ctx, centerX) {
    ...
    // Position updates IMMEDIATELY with zero transition delay
    const dir = this.isTop ? 1 : -1;
    const x1 = centerX + (this.state === 0 ? 0 : -this.spacing * dir);
    const x2 = centerX + (this.state === 0 ? this.spacing * dir : 0);

    // Snappy tactile scale on the centered dot for instant feedback
    const centerScale = 1 + (1 - this.popProgress) * 0.15;
    const r1 = this.state === 0 ? this.radius * centerScale : this.radius;
    const r2 = this.state === 1 ? this.radius * centerScale : this.radius;

    // Draw Dot 1 (Cyan)
    this.renderDot(ctx, x1, this.y, r1, this.color1);

    // Draw Dot 2 (Pink)
    this.renderDot(ctx, x2, this.y, r2, this.color2);
...
class TwoDotsGame {
  ...
  bindEvents() {
    ...
    // Touch & Click to Swap - Instant zero-delay response
    const handleTap = (e) => {
      if (e.pointerType === 'mouse' && e.button !== 0) return;
      if (this.state === 'PLAYING' || this.state === 'COUNTDOWN') {
        // Prevent click if clicking HUD buttons
        if (e.target && e.target.closest && e.target.closest('#game-hud')) return;
        this.swapDots();
      }
    };
  ...
  onMatchFail(hitY, centerDotColor) {
    ...
    try {
      ...
      console.error('[TwoDots] sendScore error:', e);
    }
