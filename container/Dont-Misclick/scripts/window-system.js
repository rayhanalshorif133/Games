...
class WindowManager {
    ...
    spawn(options = {}) {
        ...
        // Window dimensions (optimized larger size for mobile screen touch gameplay)
        let baseW = options.width || (740 + Math.floor(Math.random() * 160));
        let baseH = options.height || (420 + Math.floor(Math.random() * 90));

        const minX = 35;
        const maxX = Math.max(minX, 1080 - baseW - 35);
        const minY = 120; // below top HUD
        const maxY = Math.max(minY, 1820 - baseH - 30); // above taskbar
    ...
    spawnMisclickPenalty(misclickStageX, misclickStageY, count = 5) {
        for (let i = 0; i < count; i++) {
            ...
            const targetX = misclickStageX + Math.cos(angle) * dist - 380;
            const targetY = misclickStageY + Math.sin(angle) * dist - 210;
