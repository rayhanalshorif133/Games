...
class ClickToSurviveGame {
    ...
    bindEvents() {
        ...
        const playTutorialBtn = document.getElementById('btn-play-tutorial');
        if (playTutorialBtn) playTutorialBtn.addEventListener('click', () => this.startTutorial());
        ...
        if (skipTutorialBtn) {
            ...
            skipTutorialBtn.addEventListener('pointerdown', (e) => e.stopPropagation());
        }
    ...
    skipTutorial() {
        ...
        this.windowManager.clear();
        this.ui.showMutationAlert('⚡ GAME STARTED!', 'Survive 3 minutes without crashing!');
        for (let i = 0; i < 3; i++) {
            this.windowManager.spawn({
                x: 80 + (i * 80),
                y: 300 + (i * 280)
    ...
    spawnTutorialStep(step) {
        ...
        setTimeout(() => {
            if (winObj && winObj.closeBtn) {
                this.ui.showTutorialHand(winObj.closeBtn, step, this.tutorialTotal, winObj.x, winObj.y, winObj.w, winObj.h);
            }
        }, 80);
    }
    ...
    handleMisclick(x, y) {
        ...
        // Friendly misclick handling during interactive tutorial
        if (this.inTutorial) {
            if (window.sound) window.sound.playError();
            this.ui.triggerScreenShake();
            this.ui.showFloatingPenalty(x, y, '⚠️ Tap the red [ ✕ ] button above!');
            return;
        }
    ...
    triggerSendScore() {
        ...
        // Also submit to local scoreAPI if available
        if (window.scoreAPI && typeof window.scoreAPI.submitScore === 'function') {
            const acc = this.totalClicks > 0 ? Math.max(0, Math.round((this.score / this.totalClicks) * 100)) : 100;
            window.scoreAPI.submitScore(currentScore, {
                misclicks: this.misclicks,
                accuracy: acc
            }).catch(err => console.warn('scoreAPI submitScore error:', err));
        }
    }
